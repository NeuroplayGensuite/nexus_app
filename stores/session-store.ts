    ...
    (set, get) => ({
      ...
      setChildProfile: (profile) => {
        ...
        if (isSupabaseConfigured()) {
          saveChildProfile(profile).then((result) => {
            if (result) {
              console.log('✅ Profile synced to cloud');
            }
          }).catch((err) => {
            console.warn('⚠️ Cloud sync unavailable - data saved locally');
          });
        }
      },
      ...
      endSession: () => {
        ...
        if (currentSession) {
          ...
          // Limit to last 20 sessions to prevent localStorage quota exceeded
          ...
          // Auto-sync session to Supabase if configured (long-term storage)
          if (isSupabaseConfigured() && childProfile) {
            saveGameSession(completedSession, childProfile.id).then((result) => {
              if (result) {
                console.log(`✅ ${completedSession.gameType} synced to cloud`);
              }
            }).catch((err) => {
              console.warn('⚠️ Cloud sync unavailable - data saved locally');
            });
